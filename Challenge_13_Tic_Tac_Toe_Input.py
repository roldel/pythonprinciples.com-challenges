...
in that cell on the board. To do so, you need to translate from the string "C1"
...
single parameter which is a string of length 2 consisting of an uppercase letter and
a digit.
...
def get_row_col(directive):
    
    #isolate letter and number instructions from directive
    letter = directive[0]
    number = directive[1]
    
    #translate directive letters into index numbers for columns
    coordict = {"A": 0, "B": 1, "C": 2}
    column = coordict[letter]
    
    #Correction as grid start index is 0, not 1
    row = int(number) - 1
